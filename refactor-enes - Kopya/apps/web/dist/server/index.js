import { J as __commonJSMin, P as hasKeys, T as invariant, Y as __toESM, a as ClientOnly, i as routerContext, k as decodePath, n as useStore, q as require_react, r as useRouter, s as require_jsx_runtime, t as require_react_dom, w as createLRUCache } from "./assets/react-dom-m5QsD6Q8.js";
import { c as resolveManifestAssetLink, d as createMemoryHistory, h as isNotFound, i as createInlineCssStyleAsset, m as executeRewriteInput, o as getStylesheetHref, r as createInlineCssPlaceholderAsset, s as isInlinableStylesheet } from "./assets/atom-jEES-w-K.js";
import { a as isResolvedRedirect, i as isRedirect, r as matchContext, s as rootRouteId, t as isValidLobbyCode } from "./assets/lobby-code-Bxub0uGA.js";
...
async function getStartManifest(matchedRoutes) {
	const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-Bif0h3bX.js");
...
async function loadEntries() {
	const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
		import("./assets/router-CLs_jPS5.js"),
		import("./assets/start-COit3PSj.js"),
...
//#endregion
//#region src/shared/lib/lobby-protocol/index.ts
function parseClientMessage(value) {
	if (!isRecord(value)) return null;
	switch (value.t) {
		case "move": return "payload" in value ? {
			t: "move",
...
//#endregion
//#region src/shared/server/lobby-room.do.ts
...
var LobbyRoom = class extends DurableObject {
	...
	async fetch(request) {
		if (request.headers.get("Upgrade")?.toLowerCase() !== "websocket") return new Response("WebSocket bekleniyor", { status: 426 });
		const url = new URL(request.url);
		...
		if (this.seats.size >= MAX_PLAYERS) return rejectSocket(client, server, "full");
		if (this.seats.size === 0 && joinOnly) return rejectSocket(client, server, "not-found");
		...
		if (this.seats.size === 1) send(server, {
			t: "waiting",
			code
		});
	...
	async webSocketMessage(ws, raw) {
		...
		switch (message.t) {
			case "move":
				...
				break;
			case "rematch":
				if (this.seats.size >= 2) this.broadcast({
					t: "rematch-start",
					seed: randomSeed()
				});
				break;
	...
	startMatch() {
		...
		const names = new Array(this.seats.size).fill("");
		for (const seat of this.seats.values()) names[seat.index] = seat.name;
		for (const [ws, seat] of this.seats) send(ws, {
			t: "start",
			seed,
			names,
			you: seat.index
	...
	relayToPeer(from, message) {
		for (const ws of this.seats.keys()) if (ws !== from) send(ws, message);
	}
	broadcast(message) {
		for (const ws of this.seats.keys()) send(ws, message);
	}
	dropSeat(ws) {
		if (!this.seats.delete(ws)) return;
		this.broadcast({ t: "peer-left" });
	}
};
/** Routes a `/lobi/:code` WebSocket upgrade to that code's room instance. */
async function handleLobbyUpgrade(request, env, code) {
	const normalized = code.toUpperCase();
	if (!isValidLobbyCode(normalized)) return new Response("Lobi bulunamadı", { status: 404 });
	return env.LOBBY_ROOM.get(env.LOBBY_ROOM.idFromName(normalized)).fetch(request);
}
/** Accepts the pair just long enough to explain the rejection, then closes. */
function rejectSocket(client, server, reason) {
	server.accept();
	send(server, {
		t: "error",
		reason
...
var server_default = { async fetch(request, env) {
...
function lobbyUpgradeCode(request) {
	if (request.method !== "GET") return null;
	if (request.headers.get("Upgrade")?.toLowerCase() !== "websocket") return null;
	return new URL(request.url).pathname.match(/^\/lobi\/([^/]+)$/)?.[1] ?? null;
}
//#endregion
//#region \0virtual:cloudflare/worker-entry
var worker_entry_default = server_default ?? {};
//#endregion
export { LobbyRoom, worker_entry_default as default };
